...
define(['jquery','jqueryui','core/str'],function($,jqui,str){str.get_strings([]).done(function(){});return{init:function(){$('#mypublicpage-profile-shortform #mypublicprofile-save-btn').click(function(){var userid=$.trim($('#userid').val());var username=$.trim($('#username').val());var fullname=$.trim($('#fullname').val());var phone1=$.trim($('#phone1').val());var institution=$.trim($('#institution').val());var address=$.trim($('#address').val());var icq=$.trim($('#icq').val());var birthday=$('#birthday').val();var interests=[];var interestsElements=$('#tag-container .tag-item');if(interestsElements.length){$.map(interestsElements,function(item){interests.push($(item).text());});interests=JSON.stringify(interests);}
