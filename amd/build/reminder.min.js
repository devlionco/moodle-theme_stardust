...
define(['jquery','core/str','core/ajax','core/templates','core/notification',],function($,Str,Ajax,templates,notification){return{TEMPLATE:{reminders:{src:'theme_stardust/reminders',id:'reminders'},reminder:{src:'theme_stardust/reminder',id:'reminder'},},init:function(){var context=this;context.getReminders();$(document).on('click','#opennewreminder',function(){context.openReminderForm();});$(document).on('click','#addnewreminder',function(){context.addReminder();});$(document).on('click','.deletereminder',function(){context.delReminder($(this));});},getReminders:function(){Ajax.call([{methodname:'theme_stardust_get_reminders',args:{},done:function(res){var response=JSON.parse(res);var data=response;templates.render(this.TEMPLATE.reminders.src,data).then(function(html){$('.popover-region-notifications .popover-region-reminder-div').append(html);}.bind(this)).fail(notification.exception);if(data.counthappend>0){setTimeout(function(){var allcount=$('.popover-region-notifications .count-container').html();if(allcount){allcount=+allcount+data.counthappend;}else{allcount=data.counthappend;}
